...
LABEL_DESC_VIDEO:		Descriptor		0B8000h,	0ffffh,		DA_DRW | DA_DPL3				; 显存首地址 = 0B8000h
; GDT ------------------------------------------------------------------------------------------------------------------------------------------------------------
...
; 如果可以的话，把BIOS参数保存下来(DoZerg 20060817)，参考Linux内核0.11中setup.s文件36-105行
	push ds
	mov ax,BaseOfLoader		;参数保存的地址
	mov ds,ax
	mov ah,03h		;Read cursor，调用BIOS中断0x10，功能号ah=0x03
	xor bh,bh		;输入：bh = 页号
	int 010h		;返回：ch = 扫描开始线，cl = 扫描结束线
	mov [0],dx		;      dh = 行号(0x00是顶端)，dl = 列号(0x00是左边)
	
	mov ah,088h		;获取扩展内存大小KB，调用BIOS中断0x15，功能号ah=0x88
	int 015h		;返回：ax = 从0x100000(1M)出开始的扩展内存大小(KB)
	mov [2],ax		;若出错则CF置位，ax=出错码
	
	mov ah,0fh		;获取显卡当前模式，调用BIOS中断0x10，功能号ah=0xf
	int 010h		;返回：ah = 字符列数，al = 显示模式，bh = 当前显示页
	mov [4],bx		;0x10004(1字)存放当前页，0x90006显示模式，0x90007字符列数
	mov [6],ax		;

	mov ah,012h		;检查显示方式(EGA/VGA)并取参数
	mov bl,010h		;调用BIOS中断0x10，附加功能选择-取方式信息
	int 010h		;功能号：ah=0x12，bl=0x10
	mov [8],ax		;返回：bh = 显示状态(0x00 - 彩色模式，I/O端口=0x3dX；0x01 - 单色模式，I/O端口=0x3bX)
	mov [10],bx		;bl = 安装的显示内存(0x00-64K,0x01-128K,0x02-192K,0x03-256K)
	mov [12],cx		;cx = 显示卡特性参数
	
	sub ax,ax			;获取第一个硬盘的信息(复制硬盘参数表)
	mov ds,ax			;每个表的长度是16个字节(0x10)
	lds si,[4 * 041h]	;复制BIOS有关2个硬盘的参数表
	mov ax,BaseOfLoader	;
	mov es,ax			;
	mov di,080h			;0x90080处存放第一个硬盘的表
	mov cx,010h			;
	rep					;
	movsb				;
	...
	mov ax,BaseOfLoader
	mov es,ax
	mov di,090h			;0x090090处存放第二个硬盘的表
	mov cx,010h
	rep
	movsb
	...
	je LABEL_IS_DISK
LABEL_NO_DISK:
	mov ax,BaseOfLoader		;第二个硬盘不存在，则硬盘表清0
	mov es,ax
	mov di,090h
	mov cx,010h
	mov ax,0h
...
.Begin:
	...
	mov	eax, [esi + 04h]				;	┃
	add	eax, BaseOfKernelFilePhyAddr	;	┣ ::memcpy_ASM_FN(	(void*)(pPHdr->p_vaddr),
